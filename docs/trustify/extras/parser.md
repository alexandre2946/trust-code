...
In a mathematical function, used for example in field definition, it is possible to use the predefined functions (an object parser is used to evaluate the functions):
...
| x_AND_y | boolean AND (returns 1 if both x and y are true, else 0) |
| x_OR_y | boolean OR (returns 1 if x or y is true, else 0) |
