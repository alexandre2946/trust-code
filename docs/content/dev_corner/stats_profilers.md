@page Dev_StatsProfilers Stats and Profilers
...
@note Warning: if a custom counter already has your `std::string counter_description`, the function `statistics().create_custom_counter()` will not create another counter.
...
@note Perf and Heaptrack work best with [Hotspot](https://github.com/KDAB/hotspot), a GUI for visualizing profiling data.
