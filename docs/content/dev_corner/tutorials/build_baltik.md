@page DevTut_Baltik Build your Baltik
...
For the sequel, we recommand you the Eclipse IDE editor linked to your TRUST repository, see @ref DevTut_Eclipse.

You can now launch a first computation:

- From the `Project Explorer` tab, right click MY_BALTIK → `Debug As` → `Debug Configurations...` ⇒ C/C++ Application → Select the configuration containing your baltik's name

- In the `Main` tab: ⇒ Project: MY_BALTIK ⇒ C/C++ Application: contains path to `$exec_debug` ⇒ `Apply`
...
**With Eclipse:**
   ...
   - Build/fix/re-build your project: → `Project` and `Build project`

   - Run the test case: → `Run` and `Debug`
...
and compare the files: Cx_result.txt, PAR_Cx_result.txt.
...
For more information, go check @ref KeywordsReference and search for the Debog Keyword.
...
```
cd $TRUST_ROOT/docs/exercices/my_first_class
```

You can practice on this other tutorial: @ref DevTut_BaltikCD.
