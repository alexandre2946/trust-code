@page GG_BoundaryConditions Boundary conditions
...
| Robin_VEF | Available only with the VEF Pnc-P0 discretization. Robin flux term for Navier-Stokes equations. Uses Robin coefficients `alpha` (normal) and `beta` (tangential), and a field `flux_normal_et_tangentiel` (concatenation of normal and tangential flux components). |
