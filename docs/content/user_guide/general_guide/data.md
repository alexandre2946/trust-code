@page GG_Data How to use a data file
...
-  A **Fluide_incompressible** (incompressible_Fluid) object. This type of object is defined by its physical characteristics (its dynamic viscosity \f$\mu\f$ (keyword **mu**), its density \f$\rho\f$ (keyword **rho**), etc...).
...
-  You can search for a keyword meaning in @ref KeywordsReference
...
For more informations regarding parallel test case, go to the @ref GG_Parallel section.
...
You can find all the available **TRUST** problems at @ref GG_Problems.
...
This is useful for parallel calculation if well placed in the datafile (see section @ref GG_Parallel).

#### Allowed meshes

**TRUST** allows all types of meshes if the appropriate spatial discretization is used. See the @ref GG_SpatialSchemes section for the list of available discretizations and some advices regarding their usage.
...
-  for another format, see @ref KeywordsReference

You can have a look too at the @ref GG_PostProcessing section.
...
For more details regarding these commands, go find them in @ref KeywordsReference There, you will also find other commands, under the `interprete` section.
...
You have to specify a discretization type to run a simulation. See @ref GG_SpatialSchemes. For example, if you want to use the **VDF** discretization:
```bash
...
The time schemes available in the **TRUST** platform are summarized in the @ref GG_TemporalSchemes section.
...
More information regarding this block are given in @ref GG_Problems.

First, in this block, you will have to specify the type of equations you will solve. Your choices here depend on your problem choice, see @ref KeywordsReference
```bash
   # hydraulic problem #
...
For other types and more information see @ref KeywordsReference
...
Go check the @ref GG_Solvers section to see the options you may use.
...
In the convection block, you need to specify a spatial scheme. Have a look at the @ref GG_SpatialSchemes section for a list of the available schemes in **TRUST**.

In the sources block, you will define your source terms. You will need to go check @ref KeywordsReference to pick up the good keyword for your case. Here are some available source terms:
...
-  **Boussinesq_temperature** { ... }, defined as \f$\rho(T)=\rho(T_0)(1-\beta_{th}(T-T_0))\f$
...
To choose your *boundary_field_type* parameters, refer to @ref KeywordsReference
...
```
The post-processing options are listed in @ref GG_PostProcessing.
...
```
Some more details regarding this step are given in @ref GG_EndData.
