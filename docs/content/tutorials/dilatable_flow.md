@page Tut_DilatableFlow Dilatable flow
...
Then, open @ref KeywordsReference in another tab, as it will be useful for looking up keywords throughout this exercise.
...
- First, modify the geometry and the mesh as shown below.

![Geometry modification for the Low-Mach case](figures/low_mach.png)
...
- The goal of the calculation is to reach steady state, so remove the **tmax** keyword and change the **seuil\_statio** \f$\varepsilon\f$ value to 10 (\f$|dT/dt|<\varepsilon\f$ and \f$dt \sim 0.001s\f$, so \f$|dT|<0.01\f$).
...
- Once the calculation is finished, visualize the results using **VisIt**:
   ...
   - Display the mesh and visualize the temperature field by selecting the last time step with the slider, then plot it: `Add` → `Pseudo Color` → `TEMPERATURE_SOM_dom` → `Draw`.

   - Hide the mesh and visualize the velocity field: `Add` → `Vector` → `VITESSE_SOM_dom` → `Draw`.

   - Save a screenshot of your visualization: `File` → `Set Save options` → `File type` → `Select a type` → `Save`. An image file named `visit***` will be created in your working directory.

   - Add a second screen with `Window` → `Layout` → `1x2` and plot a horizontal temperature profile. To do so, select the temperature field, right-click and select `Mode` → `Lineout`, then define your profile with the left button. The profile should appear in the second window.

   - Close **VisIt**.

Going back to your `.data` file, replace the time scheme with an implicit time scheme such as `schema_euler_implicite`.

Use the **implicite** solver and specify the **facsec** and **facsec\_max** parameters according to the advice given in the reference manual.
