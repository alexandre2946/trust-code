@page Tut_ValidationForm Validation form
...
@note See the figure below for an example of a table plot.

![Example of a Jupyter Notebook block](figures/jupyter_table.png)

Now, we are going to add a fourth test case: `debit4` which corresponds to the `std` test case with zero initial velocity and imposed flow rate to \f$2 m^3 \cdot s ^{-1}\f$ on `periox` boundary.
