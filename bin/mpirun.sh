...
usage() {
    echo "Usage: mpirun.sh [-gdb | -valgrind [-gdb]| -valgrind_strict [-gdb]] -np nb_procs binary_path test_case nb_procs"
    exit
}
[[ $# = 0 ]] && usage
...
/usr/bin/srun)
    ...
    ;;
/usr/bin/prun)
    ${Mpirun} -vts -n "${2}" "${3}" "${4}" "${5}"
    ;;
/opt/hpmpi/bin/mpijob)
    ${Mpirun} "${3}" "${4}" "${5}"
    ;;
/opt/hpmpi/bin/mpirun)
    ${Mpirun} -srun -n "${2}" "${3}" "${4}" "${5}"
    ;;
/opt/mpi/mpibull*/bin/mpirun)
    srun -n "${2}" "${3}" "${4}" "${5}"
    ;;
*/poe)
    export MP_NODES=1
...
*)
    ...
    ############################
    MPI=OPENMPI
    if ${Mpirun} --help 2>&1 | grep -q -i mpich; then
        MPI=MPICH
    fi
    ...
    if [[ "${1}" = "-gdb" ]]; then
        ...
        if [[ "${MPI}" = "MPICH2" ]]; then
            ...
            export TRUST_GDB=1
            "${Mpirun}" "${machinefile}" "${@}"
        elif [[ ${MPI} = "OPENMPI" ]] || [[ ${MPI} = "MPICH" ]]; then
            # OpenMPI && MPICH 3:
            gdb_options=$(mktemp_)
            gdb=$(mktemp_)
            shift
    ...
    # Support de valgrind #
    #######################
    elif [[ "$(echo ${*} | grep bin/valgrind)" != "" ]]; then
        ...
        if [[ -f "${4}.err" ]]; then
            ...
            if [[ "$(grep -e 'writev failed' -e 'Address not mapped' $4.err)" != "" ]]; then
