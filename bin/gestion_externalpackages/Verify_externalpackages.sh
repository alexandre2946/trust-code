...
(cd $TRUST_ROOT/externalpackages; md5sum * */* */*/* */*/*/* 2>/dev/null | sort -k 2 | grep -v RELEASE_NOTES | grep -v conda_list_ref | grep -v README_Miniforge | grep -v channeldata | grep -v repodata.json | grep -v index.html | grep -v repodata_from_packages.json | grep -v check_packages | sed "s/ \*/  /") > md5.check
