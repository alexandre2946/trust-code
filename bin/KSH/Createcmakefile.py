...
#
SPATIAL_DISCRET = ['src/EF', 'src/IJK', 'src/PolyMAC_family', 'src/VDF', 'src/VEF', 'src/DG', 'src/Coloc']
...
if(NOT VISUAL)
    ...
    elseif (CMAKE_BUILD_TYPE STREQUAL "profiling")
        set(OPT "_opt_profiling")
    elseif (CMAKE_BUILD_TYPE STREQUAL "custom")
        set(OPT "_custom")
        include (${TRUST_ROOT}/env/Cmake.custom)
    else(CMAKE_BUILD_TYPE STREQUAL "Release")
       message(FATAL_ERROR  "Unknown build_type ${CMAKE_BUILD_TYPE} !, use -DCMAKE_BUILD_TYPE=Release,Debug,Profil,Coverage,semi_opt,profiling,custom")
    ...
    set(TRUST_HDF5_ROOT $ENV{TRUST_HDF5_ROOT})
...
if(NOT ATELIER) # Not a Baltik, TRUST itself
    ...
    if("${OPT}" STREQUAL "" AND "$ENV{TRUST_DISABLE_GTEST}" STREQUAL "0" )
        ...
        if ("$ENV{TRUST_DISABLE_MPI}" STREQUAL "0")
            # Parallel tests
            ##
            ## 2 procs
            ##
            add_test(NAME Unit_GTest_MPI_2procs COMMAND "$ENV{TRUST_ROOT}/bin/mpirun.sh" "-np 2" "$ENV{TRUST_ROOT}/exec/unit_gtest_mpi -unit")
        endif()
    endif()
