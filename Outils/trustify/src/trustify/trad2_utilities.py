...
class TRAD2Content:
    ...
    def BuildContentFromTRAD2(cls, trad2, trad2_nfo=None):
        ...
        for lin_n, l in enumerate(lines):
            ...
            if isinstance(obj, TRAD2Block):
                ...
                if curr_obj is None:
                    raise Exception(pretty_error(trad2, lin_n, f"'XD attr' (attribute line) read before main 'XD' line (header line)!"))
    ...
    def toTRAD2(self, f_nam_out):
        ...
        logger.info(f"==> Written file '{f_nam_out}.nfo'")
    ...
    def _parseXD_ADD_something(self, tag="XD_ADD_P", expected_num_args=2, f_nam="", cpp_meth="ajouter",
        ...
        if not f".{cpp_meth}" in lin_start:
            raise Exception(pretty_error(f_nam, lin_n, f"{tag} instruction put on a line where 'Param::{cpp_meth}*' method is not called!!"))
        ob, cb = lin_start.find('('), lin_start.find(')')
        params = lin_start[ob+1:cb]
        tb = params.split(',')
        if len(tb) not in [1, 2,3]:
            raise Exception(pretty_error(f_nam, lin_n, f"wrong number of arguments to Param::ajouter*() method??"))
        # Param name
    ...
    def scanOneCppLine(self, lvl, lin, curr_obj, res, f_name, lin_n):
        ...
        if f"//{XD}" in tab or f"//{XD}_ADD_P" in tab:
            raise Exception(pretty_error(f_name, lin_n, f"misformatted {XD} comment (should have space before {XD} ...)"))
        ...
        if XD in tab:
            ...
            if isinstance(obj, TRAD2Block):
                ...
                if curr_obj[lvl] is None:
                    raise Exception(pretty_error(f_name, lin_n, f"'XD attr' read before main 'XD' line!"))
