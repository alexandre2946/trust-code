...
from trustify.core.misc_utilities import (
    TrustifyException,
    TrustifyInternalError,
    TrustifyParseError,
    get_single_parser_base,
)
from trustify.core.trust_parser import TRUSTTokens, _advance_position

########################################################################


def _render_genErr_message(cls, stream, msg, attr):
    """Render the human-readable error block: error message in red, the
    "triggered in the following context" body in yellow. Coloring is
    gated by `paint()` on `colors_enabled()`, so the output is plain
    text when stdout isn't a TTY (e.g. captured by a test harness).
    """
    from trustify._color import RED, YELLOW, paint

    err = "\n" + paint(msg, RED) + "\n"
    body = "=> This error was triggered in the following context:\n"
    body += f"   Dataset: line {stream.currentLine()}  in file  '{stream.fileName()}'\n"
    s = ""
    # _infoMain is now a (project, path, line) triple — see
    # `trustify.core.trad2_pydantic._info_to_triple`. The renderer
    # shows the (relative) path and the line; the project name is
    # consumed by go-to-definition resolvers (LSP / doc generator).
    if len(cls._infoMain) >= 3:
        s = f"   Model:   line {cls._infoMain[2]}  in file  '{cls._infoMain[1]}'\n"
    if attr is not None:
        ze_cls = cls
        ...
        if attr in ze_cls._infoAttr:
            _proj, fnam, lineno = ze_cls._infoAttr[attr]
            s = f"   Model:   line {lineno}  in file  '{fnam}'\n"
    body += s
    return err + paint(body, YELLOW)


def _stream_pos(stream):
    """Best-effort 0-based ``(line, col, end_line, end_col)`` of the
    stream's current token. Returns ``(stream.currentLine() - 1, None,
    None, None)`` when contentLine/contentCol are not available (corner
    cases: consumed past EOF, synthetic stream).

    A merged multi-line quoted token (see ``_mergeQuoted``) ends on a
    later line than it starts, so the end is computed by walking the body
    via ``_advance_position`` — same as ``TRUSTStream._zeroBasedEnd`` —
    not by adding a length to the start column (audit #2)."""
    cur_line_1based = stream.currentLine()
    line0 = max(0, cur_line_1based - 1)
    # During probing, `stream.idx` is still the previous validated slot;
    # the probed token (the one we're about to flag) lives at
    # `_probeIdx - 1`. `currentTokenIdx()` routes to the right one.
    idx = stream.currentTokenIdx()
    try:
        if 0 <= idx < len(stream.contentLine):
            ...
            while i < len(tok) and tok[i] in (" ", "\t", "\n"):
                i += 1
            end_line, end_col = _advance_position(line0, col, tok[i:])
            return line0, col, end_line, end_col
    except Exception:
        pass
    # Fallback: keep end_line == line0 (the prior GenErr contract); only
    # end_col degrades to None when no token can be pointed at.
    return line0, None, line0, None
...
class Abstract_Parser:
    ...
    _infoMain = ()  #: (project, path, line) triple — see trustify.core.trad2_pydantic._info_to_triple. () means no info.

    # Set by the generated module footer for every parser subclass.
    # Provides the back-reference to the owning SchemaModule used by
    # the helper classmethods below for sibling-class resolution.
    _schema_module: ClassVar[Any | None] = None
    ...
    def __init__(self, pyd_value=None):
        ...
        if pyd_value is not None and not Builtin_Parser.IsBuiltin(pyd_value.__class__):
    ...
    def GenErr(cls, stream, msg, *, attr=None, token=None, kind):
        ...
        line0, col, end_line, end_col = _stream_pos(stream)
    ...
    def GetAllClassesFromSyno(cls, kw, stream):
        ...
        if kw.endswith("_64"):
            kw = kw[: -len("_64")]
        # `_has_class` raises TrustifyInternalError if _schema_module is None
        # (audit 5.7), so the syno_map fetch below is safe to assume non-None.
        if cls._has_class(kw):
            ret.append(cls._pyd_class(kw))
        syno_map = cls._schema_module._SYNO_ORIG_NAME
        if ret == [] and kw not in syno_map:
    ...
    def GetOneClassFromSyno(cls, kw, stream, interp=True):
        ...
        if len(root_cls) > 1:
            ...
            if interp:
                interp_cls = cls._pyd_class("Interprete")
                matches = [c for c in root_cls if issubclass(c, interp_cls)]
                assert len(matches) <= 1, (
                    f"Synonym '{kw}' matches more than one Interprete keyword, list is {matches}!!"
                )
                if len(matches):
                    return matches[0]
            if not cls._has_class(kw):
                raise cls.GenErr(
                    stream,
    ...
    def checkToken(self, key, expec, typ=str):
        ...
        """
        lst_low = self._tokens.get(
            key, TRUSTTokens()
        ).low()  # Sometimes the token has not been registered at all! (added attribute for example)
    ...
    def getBraceTokens(self, brace):
        ...
        if self.checkToken(brace, brace):
            ...
            if brace == "{":
                ...
                return ["\n}\n"]
            raise TrustifyInternalError(f"unexpected brace parameter {brace!r}; expected '{{' or '}}'")
    ...
    def WithBraces(cls):
        ...
        if cls._braces in mp:
            return mp[cls._braces]
        # Need to look at the parent
        from trustify.core.naming import _PARSER_SUFFIX

        for c in cls.__bases__:
            if c.__name__.endswith(_PARSER_SUFFIX):
                return c.WithBraces()
        raise TrustifyInternalError(
            f"{cls.__name__}: WithBraces() bottomed out — no Parser-suffixed parent class found"
        )

    @classmethod
    def _resolve_attr(cls, attr_name):
        """Look up ``attr_name`` on the owning SchemaModule. Loud failure
        when the lookup misses or when ``cls`` has no schema back-reference.
        """
        if cls._schema_module is None:
            raise TrustifyInternalError(
                f"{cls.__name__}: no _schema_module set — schema activation "
                "step was missed. See the 'Multi-schema coexistence' gotcha "
                "in CLAUDE.md (audit 5.7)."
            )
        try:
            return getattr(cls._schema_module, attr_name)
        except AttributeError:
            raise TrustifyException(f"Class '{attr_name}' not found in schema {cls._schema_module.__name__}") from None

    @classmethod
    def _pyd_class(cls, name):
        """Resolve a pydantic class by free-form TRUST keyword name."""
        from trustify.core.naming import ToPydName

        return cls._resolve_attr(ToPydName(name))

    @classmethod
    def _parser_class(cls, name):
        """Resolve a parser class by free-form TRUST keyword name."""
        from trustify.core.naming import ToParserName

        return cls._resolve_attr(ToParserName(name))

    @classmethod
    def _pyd_from_parser(cls, parser_cls):
        """Return the pydantic class paired with ``parser_cls``."""
        from trustify.core.naming import _PARSER_SUFFIX

        pyd_name = parser_cls.__name__[: -len(_PARSER_SUFFIX)]
        return cls._resolve_attr(pyd_name)

    @classmethod
    def _parser_from_pyd(cls, pyd_cls):
        """Return the parser class paired with ``pyd_cls``."""
        from trustify.core.naming import _PARSER_SUFFIX

        return cls._resolve_attr(pyd_cls.__name__ + _PARSER_SUFFIX)

    @classmethod
    def _has_class(cls, name):
        """True iff ``name`` resolves to either a pyd or parser class.

        Raises TrustifyInternalError if `_schema_module` is None — that
        is always a programmer bug (the schema activation step was
        missed; see CLAUDE.md "Multi-schema coexistence"). Silently
        returning False used to mask the activation issue behind a
        downstream "Invalid TRUST keyword" error (audit 5.7).
        """
        ...
        return hasattr(cls._schema_module, ToPydName(name)) or hasattr(cls._schema_module, ToParserName(name))

    @classmethod
    def _parser_cls_for(cls, pyd_obj):
        """Resolve the parser class paired with ``type(pyd_obj)``, routed
        through the pyd value's OWN schema affinity rather than through
        ``cls._schema_module``.

        Use this from toDatasetTokens-time code paths (where a fresh pyd
        object's ``_parser`` may be ``None`` and we need to construct
        one). Routing through ``pyd_obj``'s schema is the only way to
        stay correct when two schemas coexist in the process — the
        shared parser-base ``_schema_module`` slot only tracks the
        most-recently activated schema and would otherwise mis-resolve.

        Falls back to ``cls._schema_module`` (the activated schema) for
        pyd objects with no schema affinity declared — covers
        hand-built fixtures and any pyd that predates the affinity
        contract.
        """
        from trustify.core.naming import _PARSER_SUFFIX

        pyd_cls = type(pyd_obj)
        schema_mod = getattr(pyd_cls, "_schema_module", None) or cls._schema_module
        if schema_mod is None:
            raise TrustifyException(
                f"Cannot resolve parser for {pyd_cls.__name__}: pyd has no "
                f"_schema_module affinity and parser class has no activated schema"
            )
        try:
            return getattr(schema_mod, pyd_cls.__name__ + _PARSER_SUFFIX)
        except AttributeError:
            raise TrustifyException(
                f"Class '{pyd_cls.__name__}{_PARSER_SUFFIX}' not found in schema {schema_mod.__name__}"
            ) from None
...
class Builtin_Parser(Abstract_Parser):
    ...
    def InstanciateFromBuiltin(cls, builtin_cls):
        ...
        key = (typs[0], typs[1], ann) if len(typs) == 2 else typs[0]
        # Retrieve parser class:
        pars_cls = typ_map.get(key)
        if pars_cls is None:
            Listobj_impl_cls = cls._pyd_class("listobj_impl")  # will always exist
            # Is it a Literal?
            ...
            # List of complex (non builtin) objects
                ...
                pars_cls_nam = ann
                assert ann is not None, "Complex list types should be annotated!"
                assert cls._has_class(pars_cls_nam), (
                    f"The list parser type '{pars_cls_nam}' was not found! Should never happen."
                )
                pars = cls._parser_class(pars_cls_nam)()
            elif issubclass(typs[0], Listobj_impl_cls):
                pars = cls._parser_from_pyd(typs[0])()
            else:
                raise TrustifyException(f"Builtin type '{builtin_cls!s}' not supported!")
            return pars
        return pars_cls()
...
class BaseCommon_Parser(Abstract_Parser):
    ...
    _infoAttr: ClassVar[dict] = {}  #: same thing for class attributes (dictionnary indexed by main attr name)
    ...
    def GetAllTrustNames(cls):
        """Returns all possible names (incl. synonyms) for a keyword as a nicely formatted string"""
        pyd_cls = cls._pyd_from_parser(cls)
    ...
    def GetInfoAttr(cls, attr_nam):
        ...
        if base_cls is Abstract_Parser:
            raise TrustifyInternalError(f"{cls.__name__}: missing _infoAttr entry for {attr_nam!r}")
...
class ConstrainBase_Parser(BaseCommon_Parser):
    ...
    def __init__(self, pyd_value=None):
        ...
        clsnam = TRUSTTokens(low=[], orig=[" " + self.__class__._pyd_from_parser(self.__class__).__name__.lower()])
        self._tokens = {
            "{": opbr,  #: For a ConstrainBase we might need opening and closing brace. By default those
            "}": clobr,  # are simple '{' and '}' followed by a line return. If ReadFromTokens() was
            "cls_nam": clsnam,
        }  # invoked to build the object this will respect the initial input (with potentially more spaces)
    ...
    def _InvertAttrSyno(cls):
        ...
        def append_synos(all_syno, one_cls):
            ...
            pyd_cls = one_cls._pyd_from_parser(one_cls)
    ...
    def _parseAndSetAttribute(self, stream, attr_nam, attr_cls, flavor=""):
        ...
        cls = self.__class__
        cls.Dbg(f"@FUNC@ attr_nam '{attr_nam}' attr_cls: '{attr_cls!s}' {flavor}")
        if Builtin_Parser.IsBuiltin(attr_cls):
            ...
            pars_attr_cls = cls._parser_from_pyd(attr_cls)
            # Parse child attribute:
            val_attr = pars_attr_cls.ReadFromTokens(stream)

        # Set the attribute value
        cls.Dbg(f"@FUNC@ setting {flavor} attribute '{attr_nam}' with val '{val_attr!s}'")
    ...
    def _readFromTokens_no_braces(self, stream):
        ...
        stream.pos()
        _invert_attr_syno, ca, nams = cls._InvertAttrSyno(), cls._GetAttributeList(), cls.GetAllTrustNames()
        ...
        while attr_idx < len(ca):
            ...
            while attr_idx < len(ca) and cls.IsOptional(ca[attr_idx][0]):
                ...
                stream.save(
                    "BEFORE_OPT"
                )  # Save the stream state at this point in case we need to rewind for mis-parsed attribute
                try:
                    self._parseAndSetAttribute(stream, attr_nam, attr_cls, "(optional)")
                    opt_match = True  # OK, optional attribute match!
                    attr_idx += 1
                    break
                except (TrustifyException, TRUSTEndOfStreamException):
    ...
    def _ReadClassName(cls, stream):
        ...
        for r in cls.GetAllClassesFromSyno(kw, stream):  # GetAllClassesFromSyno will also check if valid TRUST keyword
            me_as_pyd = cls._pyd_from_parser(cls)
    ...
    def ReadFromTokens(cls, stream, pars_cls=None):
        ...
        if pars_cls._read_type:
            ...
            ze_pars_cls = cls._parser_class(cls_nam)
        ...
        val_cls = cls._pyd_from_parser(ze_pars_cls)  # from 'pars_cls', not from 'cls'
        val = val_cls()
    ...
    def _getClsTokens(self):
        ...
        cls = self.__class__
        # When a class-name token was captured at read time (either by the
        # polymorphic-dispatch path in ReadFromTokens, or by the
        # `pars_cls._read_type==False` branch of Dataset_Parser._handle_interprete
        # that still stores cls_nam), echo it back regardless of the instance's
        # class-level _read_type. This lets concrete classes that have
        # _read_type=False (Milieu / Mor_eqn / Pb_base sweep in hacks.py) still
        # round-trip their discriminator when they were reached via base-class
        # dispatch in an attribute context, e.g. fluide0 of type fluide_base in
        # Fluide_Diphasique.
        #
        # NB: ConstrainBase_Parser.__init__ pre-seeds _tokens["cls_nam"] with a
        # synthetic placeholder (low=[], orig=[" <classname>"]) used by the
        # fallback writeback. We need to tell the synthetic seed apart from a
        # real stream token — the seed has an empty low(), a real read has at
        # least one entry. Check truthiness of low() (= list non-empty), not
        # equality against "" (lists are never equal to strings in Python, so
        # ".low() != ''" is uselessly always True).
    ...
    def _extendWithAttrTokens(self, tok_lst, attr_nam, attr_val):
        ...
        if Builtin_Parser.IsBuiltin(attr_cls):
            # Is this a new attr?
            if attr_nam not in self._leafParsers:
            ...
            if attr_pars is None:
                attr_pars = self._parser_cls_for(attr_val)()
    ...
    def _toDatasetTokens_braces(self):
        ...
        for attr_nam in self._attrInOrder + extra_attr:
            ...
            tok = self._tokens[attr_nam].orig() if attr_nam in self._tokens else ["\n" + attr_nam + " "]
            s.extend(tok)
...
class ListOfBase_Parser(Builtin_Parser):
    ...
    def _WithComma(cls):
        ...
        for c in cls.__bases__:
            if c.__name__.endswith(_PARSER_SUFFIX):
                return c._WithComma()
        raise TrustifyInternalError(
            f"{cls.__name__}: _WithComma() bottomed out — no Parser-suffixed parent class found"
        )
    ...
    def initItemParserType(self):
        ...
        if self.__class__._itemParserType is None:
            self.__class__._itemParserType = self.__class__._parser_from_pyd(self._itemType)
    ...
    def parseAndAppendItem(self, stream, lst):
        ...
        if stream.pos() == pos_0:
            pyd_typ = self.__class__._pyd_from_parser(self.__class__)
    ...
    def readFromTokensBuiltin_impl(self, stream):
        ...
        """
        cls = self.__class__
        cls.Dbg("@FUNC@ about to read list ...")
        val = []

        self.initItemParserType()

        if cls._MustReadSize():
            cls.Dbg("@FUNC@ reading list size ...")
            n = self.readListSize(stream)
            self._tokens["len"] = stream.lastReadTokens()
        elif not cls.WithBraces():
            err = _render_genErr_message(
                cls, stream, "schema bug: list class has neither brace policy nor size info", None
            )
            raise TrustifyInternalError(err)
        ...
        if cls.WithBraces():
            ...
            while tok != "}":
                ...
                if withComma and tok != "}":
                    ...
                    self._tokens[f",{i}"] = stream.lastReadTokens()
                    tok = stream.probeNextLow()
                i += 1
            cls.ConsumeBrace(stream, "}")
            self._tokens["}"] = stream.lastReadTokens()
    ...
    def extendWithSize(self, ret):
        ...
        ln = self._tokens["len"].orig() if self.checkToken("len", len_as_str) else [" " + len_as_str]
        ret.extend(ln)

    def getItemTokens(self, _index, elem_pyd):
        ...
        if pars is None:
            pars_cls = self._parser_cls_for(elem_pyd)
        ...
        s = []
        # `_getClsTokens()` on the item now (post-fix) emits the captured
        # cls_nam token by itself when the item was read polymorphically
        # (cls_nam.low() is truthy, i.e. a real stream token), so the
        # list-side emission below would double up. Only emit here when the
        # item is going to be silent: its cls_nam slot still holds the
        # synthetic seed (low()=[]) AND its _read_type is False.
        item_emits_cls = bool(pars._tokens.get("cls_nam", TRUSTTokens()).low()) or pars._read_type
        if self._outputItemType and not item_emits_cls:
    ...
    def toDatasetTokens(self):
        ...
        if self._MustReadSize():  # if size must be read, it must be written out too ...
            ...
            if isinstance(self, AbstractSizeIsDim) and len(self._pyd_value) != dim:
                raise TrustifyException(
                    f"List '{self._pyd_value}' has a number of items which do not match problem dimension (={dim})!"
                )
        ...
        for i, v in enumerate(self._pyd_value):
            ...
            if self._WithComma() and i != len(self._pyd_value) - 1:
                # yes, the ',' token might be different after each element (comments etc...):
                com = self._tokens.get(f",{i}", dft_com)
                ret.extend(com.orig())
        if self.WithBraces():
            br = self.getBraceTokens("}")
            ret.extend(br)
...
class ListOfBuiltin_Parser(ListOfBase_Parser):
    ...
    def parseAndAppendItem(self, stream, lst):
        ...
        # Reset debug info (empty-info default is () — see Abstract_Parser._infoMain).
...
class Read_Parser(Interprete_Parser):
    ...
    _read_type = True
    _infoMain: ClassVar[list] = ["<trustify builtin>", -1]
    _infoAttr: ClassVar[dict] = {"identifier": ["<trustify builtin>", -1], "obj": ["<trustify builtin>", -1]}

    def __init__(self, pyd_value=None):
        Interprete_Parser.__init__(self, pyd_value=pyd_value)
        # Set per parse by Dataset_Parser via `ReadFromTokens(ze_type=...)`.
        # Instance attribute, not class attribute (audit 4.2): two
        # concurrent parses in the same process must not race on a
        # single shared slot.
        self._ze_type = None

    @classmethod
    def ReadFromTokens(cls, stream, ze_type=None):
        """Override BaseCommon_Parser.ReadFromTokens to thread `ze_type`
        (resolved by Dataset_Parser from the forward declaration) onto
        the freshly constructed parser instance before invoking the
        readFromTokens_impl that needs it."""
        pars = cls()
        pars._ze_type = ze_type
        pars._pyd_value = pars.readFromTokens_impl(stream)
        if not Builtin_Parser.IsBuiltin(pars._pyd_value.__class__):
            pars._pyd_value._parser = pars
        return pars._pyd_value
    ...
    def readFromTokens_impl(self, stream):
        ...
        """
        val = self.__class__._pyd_from_parser(self.__class__)()
        ...
        # try/finally so a parse error in the inner ReadFromTokens does not
        # leave the class-level `_read_type` stuck at False for the rest of
        # the process (audit 4.1).
        old_read_type = self._ze_type._read_type
        self._ze_type._read_type = False
        try:
            val.obj = self._ze_type.ReadFromTokens(stream)
        finally:
            self._ze_type._read_type = old_read_type
    ...
    def toDatasetTokens(self):
        ...
        if tok_obj_pars is None:
            tok_obj_pars = self._parser_cls_for(self._pyd_value.obj)()
        tok_obj_pars._pyd_value = self._pyd_value.obj
        # WARNING! Here we must skip class reading, so temporarily switch this off
        # (similar to what is done in ReadFromTokens()). try/finally per audit 4.1.
...
class Dimension_Parser(Interprete_Parser):
    ...
    _infoMain: ClassVar[list] = ["<Dimension>", -1]
    _infoAttr: ClassVar[dict] = {"dim": ["<builtin>", -1]}
    ...
    def ReadFromTokens(cls, stream):
        ...
        try:
            new_defbord_cls = cls._pyd_class(new_defbord_nam)
            defs_bords = [cls._pyd_class(n) for n in ["Defbord", "Defbord_2", "Defbord_3"]]
        except TrustifyException:
            new_defbord_cls = None
            defs_bords = []
        # Dynamically change all attribute types using this:
        for c in cls._schema_module._all_constrain_base_pyd if cls._schema_module else []:
            for k, fld_nfo in c.model_fields.items():
                ...
                if typ_attr in defs_bords:
                    ...
                    pars_cls = cls._parser_from_pyd(c)
                    pars_cls._attributeList = None
                    pars_cls._attributeSynos = None
        return val
...
class Declaration_Parser(ConstrainBase_Parser):
    ...
    _infoMain: ClassVar[list] = ["<trustify builtin>", -1]
    _infoAttr: ClassVar[dict] = {"identifier": ["<trustify builtin>", -1], "ze_type": ["<trustify builtin>", -1]}
    ...
    def readFromTokens_impl(self, stream):
        ...
        decl = self.__class__._pyd_from_parser(self.__class__)()  # a Declaration object
        ...
        self._pars_cls = self.__class__._parser_from_pyd(root_cls)
        self._tokens["cls_nam"] = stream.lastReadTokens()
        ident = Chaine_Parser.ParseOneWord(stream)
        # Basic checks on the identifier:
        if ident.startswith(("{", "}")) or ident[0].isnumeric():
            raise self.GenErr(
                stream,
    ...
    def toDatasetTokens(self):
        """Override - see BaseCommon_Parser"""
        s = []
        # An `export` prefix was captured by Dataset_Parser.ReadFromTokens for
        # declarations sourced from `export <Type> <name>` lines (sub-files
        # included via Read_file whose name must escape to the parent scope —
        # see Interprete_bloc::lire). Emit it back so the round-trip preserves
        # the original token.
        if "export" in self._tokens:
            s.extend(self._tokens["export"].orig())
        ze_type = self._pyd_value.ze_type
        # Tell the synthetic seed from ConstrainBase_Parser.__init__
        # apart from a real stream read by truthiness of `.low()` (the
        # seed has low=[], a real read has at least one entry). Same
        # anti-pattern as the previously-fixed _getClsTokens at line 872:
        # `.low() != ""` compared a list to a string, always True, so
        # the synthesized fallback below was dead code for any pyd
        # constructed programmatically rather than parsed from a .data
        # file (audit 1.2).
        if self._tokens.get("cls_nam", TRUSTTokens()).low():
        ...
        s.extend(cn)
        identif = self._pyd_value.identifier
        tok_id = self._tokens["identifier"].orig() if self.checkToken("identifier", identif) else [" " + identif]
        s.extend(tok_id)
        return s
...
class Dataset_Parser(ListOfBase_Parser):
    ...
    def _prepare_read_instruction(self, stream):
        """Private. When encountering a read instruction we look up
        the parser class for the identifier from the forward
        declaration table and return it together with the identifier.
        The caller threads `ze_cls` into the eventual
        `Read_Parser.ReadFromTokens(stream, ze_type=ze_cls)` call so
        the type lives on that parser instance (audit 4.2 — must not
        leak through a class attribute).
        """
        ...
        if identif not in self._pyd_value._declarations:
            raise self.GenErr(
                stream,
        ...
        self.Dbg(f"@FUNC@  'Read' instruction: Identifier: '{identif}' - Read_Parser will read type '{ze_cls}'")
        stream.restore("READ_INSTR")
        return identif, ze_cls

    def _handle_interprete(self, tok, stream, ds, pars_cls, read_ze_type=None):
        """Private. Invoke the ReadFromTokens method for a TRUST keyword triggering the parsing of the corresponding block in
        the dataset.

        `read_ze_type` is forwarded into `Read_Parser.ReadFromTokens` when
        the keyword is `read|lire` (resolved by `_prepare_read_instruction`
        from the forward declaration). Other interprete classes ignore it.
        """
        ...
        if pars_cls._read_type:
            ...
            if read_ze_type is not None:
                inst = pars_cls.ReadFromTokens(stream, ze_type=read_ze_type)
            else:
    ...
    def ReadFromTokens(cls, stream):
        ...
        ds = cls._pyd_class("Dataset")()  # A Dataset object (pydantic)
        pars = cls(ds)  # A Dataset_Parser object
        # TRUST allows a bare `{ ... }` block at top level
        # (Interprete_bloc::lire creates a new sub-Interprete_bloc when it
        # reads `{`). The contents are parsed with the same top-level
        # dispatch — forward declarations and interpretes — and `}` ends
        # the sub-block. trustify keeps the entries flat in `ds.entries`
        # (no scoping enforced) and remembers each `{` / `}` token with
        # the entry index it appeared at, so write-back re-emits the
        # braces in place. Each entry of `_block_marks` is
        # `(entry_index, brace_str, TRUSTTokens)`; entries with the same
        # `entry_index` are emitted in the order they were read.
        pars._block_marks = []

        cls._pyd_class("interprete")
        while not stream.eof():
            tok = stream.probeNextLow()
            cls.Dbg(f"@FUNC@ probing tok '{tok}'")

            # Top-level `{` / `}` grouping (mesh-prep blocks etc., see
            # Interprete_bloc.cpp:151-158).
            if tok in ("{", "}"):
                stream.validateNext()
                pars._block_marks.append((len(ds.entries), tok, stream.lastReadTokens()))
                continue

            # `export <Type> <name>` — TRUST-specific prefix on forward
            # declarations (Interprete_bloc::lire). When a sub-file is
            # included via `Read_file <file> ;`, `export` flags the
            # declared object to be added to the parent Interprete_bloc's
            # scope instead of the sub-file's local one — without this
            # flag the name does not survive the sub-file's parse, and
            # the parent dataset can't reference it after Read_file.
            #
            # trustify treats the whole `export <Type> <name>` line as a
            # regular forward declaration (so `ds._declarations` carries
            # `<name>` and subsequent `Lire <name> { ... }` resolves), with
            # the `export` token stashed on the Declaration so it round-
            # trips byte-identical on write-back.
            export_tok = None
            if tok == "export":
                stream.validateNext()  # consume 'export'
                export_tok = stream.lastReadTokens()
                tok = stream.probeNextLow()
            ...
            read_ze_type = None
            if is_read:
                cls.Dbg("@FUNC@ 'read' keyword encountered")
                read_key, read_ze_type = pars._prepare_read_instruction(stream)

            # Extract original Pydantic class from keyword
            root_cls = cls.GetOneClassFromSyno(tok, stream, interp=True)
            pars_entry_cls = cls._parser_from_pyd(root_cls)
            ...
            if issubclass(pars_entry_cls, Interprete_Parser):
                ...
                pars._handle_interprete(tok, stream, ds, pars_entry_cls, read_ze_type=read_ze_type)
                ...
                if export_tok is not None:
                    decl._parser._tokens["export"] = export_tok
                cls.Dbg(f"@FUNC@ fwd decl - '{decl.identifier}' with class '{decl.ze_type.__name__}'")
                ds._declarations[decl.identifier] = [
                    decl,
                    -1,
                ]  # -1 because for now positionof the real instanciation in the dataset is not known
    ...
    def toDatasetTokens(self):
        ...
        s = []
        # Walk `_block_marks` (`{` / `}` tokens captured at top level by
        # ReadFromTokens) in their original insertion order so empty
        # `{ }` blocks and nested `{ { } }` come back in the right order.
        # Each mark has an `entry_index`; marks with the same index are
        # flushed right before that entry is written.
        marks = list(getattr(self, "_block_marks", []))
        next_mark = 0
        for i, v in enumerate(self._pyd_value.entries):
            while next_mark < len(marks) and marks[next_mark][0] == i:
                s.extend(marks[next_mark][2].orig())
                next_mark += 1
            item_pars_cls = self._parser_cls_for(v)
        ...
        # Trailing marks (e.g. a `}` after the last entry).
...
class Bloc_lecture_Parser(ConstrainBase_Parser):
    ...
    def ReadFromTokens(cls, stream):
        ...
        val = cls._pyd_from_parser(cls)()
        # Attach this parser to it:
        val._parser = cls()
        tok = stream.probeNextLow()
        tokens = []
        cls.ConsumeBrace(stream, "{")
        tokens.append(stream.lastReadTokens())
        cnt_brace = 1
        while cnt_brace != 0:
            tok = stream.probeNextLow()
            if tok == "{":
            ...
            if tok == "}":
                cnt_brace -= 1
                if cnt_brace == 0:
                    break  # without validating token so that it is consumed by ConsumeBrace()
            stream.validateNext()
            # !!!!!!
        ...
        cls.ConsumeBrace(stream, "}")
        tokens.append(stream.lastReadTokens())
        joined = TRUSTTokens.Join(tokens)
        s = "".join(joined.orig())  # orig(), not low() see comment above!
        # Store final result:
        val.bloc_lecture = s
        val._parser._tokens["val"] = joined
        return val
...
class Int_Parser(Builtin_Parser):
    ...
    def toDatasetTokens(self):
        ...
        ret = self._tokens["val"].orig() if self.checkToken("val", self_as_str, int) else [" " + self_as_str]
        return ret
...
class Float_Parser(Builtin_Parser):
    ...
    def toDatasetTokens(self):
        ...
        ret = self._tokens["val"].orig() if self.checkToken("val", self_as_str, float) else [" " + self_as_str]
        return ret
...
class Flag_Parser(Builtin_Parser):
    ...
    def readFromTokensBuiltin_impl(self, _stream):
