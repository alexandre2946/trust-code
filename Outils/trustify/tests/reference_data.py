...
def buildCurlyExpec(mod, bidon=True):
    ...
    """
    LM = mod.Read_med_bidon if bidon else mod.Read_med
    exp = LM()
...
def buildForwardExpec(mod):
    ...
    exp0.ze_type = mod.Read_med_bidon
    exp0.identifier = "rmed"
    exp1 = mod.Declaration()
    exp1.ze_type = mod.Coucou
    exp1.identifier = "cb"
    exp2 = mod.Read()
...
def buildMinimalExpec(mod):
    ...
    exp0 = FD()
    exp0.ze_type = UF
    exp0.identifier = "gravite"
    exp1 = FD()
    exp1.ze_type = N
    exp1.identifier = "coucou"
    # Second item reading of the uniform field:
    u = UF()
    u.val = [28, 32]
    exp2 = RD()
    exp2.identifier = "gravite"
    exp2.obj = u
    # Third item is a nom:
    n = N()
    n.mot = "toto"
    exp3 = RD()
    exp3.identifier = "coucou"
    exp3.obj = n
...
def buildBordExpec(mod):
    ...
    exp.nom = "periox"
    db = DB()
    db.dir, db.eq, db.pos = "x", "=", 0.0
    db.pos2_min, db.inf1, db.dir2, db.inf2, db.pos2_max = 0.0, "<=", "y", "<=", 2.0
    exp.defbord = db
    return exp
...
def buildSolveurPressionExpec(mod):
    """Reference data for
    petsc cholesky { cli { c est un blOc leCtuRe } }
    """
    P = mod.Petsc
    Chol = mod.Solveur_petsc_cholesky
    Opt = mod.Solveur_petsc_option_cli
    p = P()
    chol = Chol()
    opt = Opt()
    # Note the space to start with:
    opt.bloc_lecture = " { c est un blOc leCtuRe }"
    chol.cli = opt
    p.solveur = chol
    return p
