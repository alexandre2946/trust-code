...
from tests.test_rw_elementary import TestCase as ElementaryTestCase
from trustify import Dataset_Parser, TrustifyParseError
from trustify.core import base
...
class TestStructuredFields(ElementaryTestCase):
    ...
    def test_location_skips_blanks_and_comments_before_bad_token(self):
        # Blanks + a comment sit between the last valid token and the
        # bad one. The reported location must point at the bad token
        # itself, not at the slot just after `dimension 2`. (Probe-mode
        # bug — `_probeIdx - 1` carries the real location while
        # `stream.idx` is still stale.)
        snippet = "dimension 2\n\n\n\n# stray comment #\n\nzzznotakeyword\n"
        err = self._parse_and_capture(snippet)
        self.assertEqual(err.kind, "invalid-keyword")
        self.assertEqual(err.token, "zzznotakeyword")
        # 0-based: `zzznotakeyword` is on the 7th source line.
        self.assertEqual(err.line, 6)
        self.assertEqual(err.col, 0)
        self.assertEqual(err.end_col, len("zzznotakeyword"))

    def test_location_skips_blanks_before_bad_brace(self):
        # Same probe-mode trap, but through ConsumeBrace: blanks +
        # newlines separate the keyword from a token that should have
        # been `{`.
        snippet = "read_med_bidon\n\n\nnot_a_brace\n"
        err = self._parse_keyword_and_capture(snippet)
        self.assertEqual(err.kind, "expected-brace")
        ...
        self.assertEqual(err.col, 0)
        self.assertEqual(err.end_col, len("not_a_brace"))
    ...
    def test_dimension_undefined(self):
        ...
        with self.assertRaises(TrustifyParseError) as cm:
            self.builtin_test(
                typing.Annotated[list[float], "size_is_dim"],
                "1.0 2.0 3.0",
                simplify=False,
            )
