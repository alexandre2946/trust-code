...
build_and_test_mc()
{
    ...
    #PL: Some issues with C++20 build on is234617-intel25 - Ubuntu24/intel 25 et is157083-clang Fedora32/clang++ 20.1.1 et is245205-clang (Fedora43/clang++ 21.1.8)
    #OPTIONS="$OPTIONS -DSALOME_CXX_STANDARD=${TRUST_STDCPP#c++}"
    OPTIONS="$OPTIONS -DSALOME_CXX_STANDARD=14"
    #INT64 management:
    if [ "$TRUST_INT64" = "1" ]
    then
